...
public class FaltanteInventarioDao extends GenericDao<FaltanteInventarioEt, Long> implements IFaltanteInventarioDao {
	...
	@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
	public FaltanteInventarioEt getFaltanteInvByCheck(CheckListEjecucionEt checkListEje) throws EntidadNoEncontradaException {
		sql = new StringBuilder("FROM FaltanteInventarioEt o ");
		sql.append(" WHERE o.estado  = :estado ");
		sql.append(" AND o.checkListEjecucion = :checkListEje ");
		TypedQuery<FaltanteInventarioEt> query = em.createQuery(sql.toString(), FaltanteInventarioEt.class);
		query.setParameter("estado", EstadoEnum.ACT);
		query.setParameter("checkListEje", checkListEje);
		List<FaltanteInventarioEt> result = query.getResultList();
...
//		}
	...
	public String generar(Long idFaltanteInv, Long idUsuario) {
		StoredProcedureQuery query = this.em.createNamedStoredProcedureQuery("getGenerarFaltInvResumen");
