...
public class InventarioFaltanteBean extends BaseBean implements Serializable {
	...
	public void redireccionar() {
		String pagina = "";
		FaltanteInventarioEt existeFaltanI;
		try {
			UsuarioEt usuario = appMain.getUsuario();
			List<FaltanteInventarioEt> faltanteInventarios = iFaltanteInventarioDao.getFaltanteInveEjecutando(usuario);
			if (faltanteInventarios != null && !faltanteInventarios.isEmpty()) {
				for (FaltanteInventarioEt faltanteInventarioC : faltanteInventarios) {
					faltanteInventarioC.setEjecutando(false);
					iFaltanteInventarioDao.guardarFaltanteInv(faltanteInventarioC, usuario);
				}
			}
			if (checkListEjecucionSeleccionado != null) {
				checkListEjecucionSeleccionado.setEjecutando(true);
				FacesContext contex = FacesContext.getCurrentInstance();
				pagina = "/PrimaxEvaluacionPruebas/pages/planificacion/pln_017.xhtml";
				...
				existeFaltanI.setEjecutando(true);
				iFaltanteInventarioDao.guardarFaltanteInv(existeFaltanI, usuario);
