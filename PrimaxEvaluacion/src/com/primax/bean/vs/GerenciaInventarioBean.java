...
public class GerenciaInventarioBean extends BaseBean implements Serializable {
	...
	public void buscar() {
		try {
			...
			for (PlanAccionInventarioTipoEt tipoInv : planAccionInv.getPlanAccionInventarioTipo()) {
				if (tipoInv.isEnEjecucion()
						&& tipoInv.getEstadoPlanAccionInv().equals(EstadoPlanAccionInvEnum.PENDIENTE)) {
					planAccionInvTipo = tipoInv;
					if (planAccionInvTipo.getPlanAccionInvCategoria() == null
							|| planAccionInvTipo.getPlanAccionInvCategoria().isEmpty()) {
	...
	public void guardar() {
		...
		try {
			...
			PlanAccionInventarioEt planAccionInvC = iPlanAccionInventarioDao
					.getPlanAccionInventarioById(planAccionInv.getIdPlanAccionInventario());
		...
		try {
			...
			for (PlanAccionInvCategoriaEt planAccionInvCat : planAccionInvTipo.getPlanAccionInvCategoria()) {
				if (planAccionInvCat.getComentarioPlanAccion() == null) {
					mensaje = "Por favor ingresar plan de acción de la categoría " + " "
							+ planAccionInvCat.getCategoriaInventario().getDescripcion();
					break;
				}
			}
	...
	public void retroceder() {
		...
		try {
			...
			pagina = "/PrimaxEvaluacion/pages/reporte/rpt_017.xhtml";
		...
		try {
			...
			if (!planAccionInvTipo.getPlanAccionInvCategoria().isEmpty()) {
				for (PlanAccionInvCategoriaEt planAccionInvCategoria : planAccionInvTipo.getPlanAccionInvCategoria()) {
					if (planAccionInvCategoria.getCategoriaInventario()
							.getIdCategoriaInventario() == categoriaInvSeleccionada.getIdCategoriaInventario()) {
						showInfo("Categoría ya se encuentra agregada en el plan de acción.", FacesMessage.SEVERITY_INFO,
								null, "");
						return;
					}
				}
