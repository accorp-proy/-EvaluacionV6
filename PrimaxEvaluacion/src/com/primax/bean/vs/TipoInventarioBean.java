...
public class TipoInventarioBean extends BaseBean implements Serializable {
	...
	public void guardar() {
		...
		try {
			...
			showInfo("Informaci�n Grabada con �xito ", FacesMessage.SEVERITY_INFO);
	...
	public String validacionGuardar(TipoInventarioEt tipoInventario) {
		...
		try {
			if (tipoInventario.getDescripcion() == null) {
				mensaje = "Por favor ingresar descripci�n";
				return mensaje;
			}
			if (tipoInventario.getCodigo() == null) {
				mensaje = "Por favor ingresar c�digo";
				return mensaje;
			}

		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("Error :M�todo validacionGuardar " + " " + e.getMessage());
		}
		return mensaje;
	}
